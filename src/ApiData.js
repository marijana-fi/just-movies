...
const makeRequest = (path, query = "", page = "", sort = "") =>
   axios.get(`${BASE}${path}${API_KEY}${query}${sort}${page}&region=US`).then(success, fail);
...
const api = {
   ...
   loadMore(path, searchQuery, page, genreId) {
      return makeRequest(path, `&query=${searchQuery}&with_genres=${genreId}`, page);
   },
   movie(path) {
   ...
   moviesByGenre(id, sortValue = "popularity.desc") {
      return makeRequest("discover/movie", `&with_genres=${id}`, "", `&sort_by=${sortValue}`);
   }
};
