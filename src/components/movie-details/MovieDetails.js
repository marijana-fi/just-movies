...
class MovieDetails extends React.Component {
   ...
   static getDerivedStateFromProps(prevProps) {
      if (prevProps.movieData.favorites.some(item => item.id === +prevProps.id)) {
   ...
   render() {
      ...
      if (!this.state.loading) {
         ...
         director = credits.crew.filter(crewMember => {
            return crewMember.department === "Directing" && crewMember.job === "Director";
         });
         ...
         <div className="movie-page">
            <div className="container">
               <div className="video-wrap">
                  <div className={this.state.playVideo ? "open" : "close"}>
                     {trailer ? (
                        <ReactPlayer
                     ...
                     <button className="close-trailer" aria-label="Close trailer" onClick={this.handleTrailerClose}>
                        &times;
                     </button>
                  </div>
               </div>
               ...
               <div className="text-wrap">
                  ...
                  <h5>
                     Director: <span>{director.map(crew => crew.name)}</span>
                  </h5>
                  <h5>
                     Writers: <span>{[...new Set(writer.map(crew => `${crew.name}, `))]}</span>
                  </h5>
                  <section className="cast">
                     ...
                     <ul className="cast-wrap">
                        {credits.cast.slice(0, 6).map(castMember => {
                           return (
                              <li key={castMember.cast_id} className="cast-card">
                                 <div className="cast-img">
                                    <Img
                                       ...
                                       alt={castMember.name}
                                       loader={<Loader />}
                                       unloader={<h5>no image to display</h5>}
                                    />
                                 </div>
                                 <div className="cast-info">
                                    <h4 className="name">{castMember.name}</h4>
                                    <h6 className="character">{castMember.character}</h6>
                                 </div>
                              </li>
                           );
                        })}
